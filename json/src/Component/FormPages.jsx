...
const FormPage = () => {
  ...
  const OnAddButton = () => {
    if (validateForm()) {
      ...
      navigate('/ConformationPage', { state: inputDetails });
    }
  };
  console.log('v', enteredValue)
  // useEffect(()=>{
  //   if(data){
...
import { increment } from "../Store/redux/counterSlice";
import { useSelector } from "react-redux";
import { clearForm, updateFormDetails } from "../Store/redux/formPageSlice";


const FormPage = () => {
  const formPageDetails = useSelector((store) => store.formDetails);
  // const [firstName, setFirstName] = useState('');
  // const [lastName, setLastName] = useState('');
  // const [gender, setGender] = useState('');
  // const [qualification, setQualification] = useState('');
  ...
  const handleInputChange = (e) => {
    ...
    dispatch(updateFormDetails({name,value}));
  };
  const OnClickClear = () => {
    dispatch(clearForm())
    // setFirstName('');
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formPageDetails.firstname||formPageDetails.firstname==='') newErrors.firstname = "First name is required";
    //if (!lastname) newErrors.lastname = "Last name is required";
    // if (!gender) newErrors.gender = "Gender is required";
    // if (!qualification) newErrors.qualification = "Qualification is required";
    // if (!email) {
  ...
  const OnAddButton = (e) => {
    if (validateForm()) {
      ...
      //   }
      // ];
      navigate('/ConformationPage', { state: formPageDetails });
    }
  };
  console.log('v', enteredValue)
  // useEffect(()=>{
    ...
    <div className="container-fluid">
      <div className="container">
        <div className="row">
          <div>
            <label className="col-2">FirstName:</label>
            <input type="text" className="col-4" value={formPageDetails.firstname} name="firstname" onChange={handleInputChange} />
            {/* {errors.formPageDetails.firstname && <span style={{ color: 'Red' }}>{errors.formPageDetails.firstname}</span>} */}
          </div>
          <div>
            <label className="col-2">LastName:</label>
            <input type="text" className="col-4" value={formPageDetails.lastname} name="lastname" onChange={handleInputChange} />
            {/* {errors.formPageDetails.lastname && <span style={{ color: 'Red' }}>{errors.formPageDetails.lastname}</span>} */}
          </div>
        </div>
        {/* <div className="row">
          <div>
